...
class AddForm extends React.Component {
  componentDidMount() {
    ...
    this.props.dispatch(initialize("addArticle", initialFormData));
  }
...
AddForm = reduxForm({
  form: "addArticle" // имя формы в state (state.form.post)
})(AddForm);

AddForm = connect(
  state => ({ isAddingArticle: state.reducer.isAddingArticle }),
  dispatch => ({
